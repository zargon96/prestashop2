...
class MbeRatesCacheHelper extends ObjectModel
{
    ...
    public static function get($id_cache, $id_cart) {
        ...
        $sql->where('id_cache = "' . pSQL($id_cache) . '" AND id_cart = "' . pSQL($id_cart) . '"');
    ...
    public static function store($id_cache, $id_cart, $response) {
        if (self::exists($id_cache, $id_cart)) {
            ...
            return Db::getInstance()->update(self::$definition['table'], $data, 'id_cache = "' . pSQL($id_cache) . '" AND id_cart = "' . pSQL($id_cart) . '"');
        }
    ...
    public static function clearByCartId($id_cart) {
        return Db::getInstance()->delete(self::$definition['table'], 'id_cart = "' . pSQL($id_cart) . '"');
    }
}
