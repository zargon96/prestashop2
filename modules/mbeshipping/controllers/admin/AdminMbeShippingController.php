...
class AdminMbeShippingController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        o.`reference` , o.`id_order` AS `buttons`, o.`id_order` as status, IF(mso.`id_order` IS NULL,\'null\',mso.`is_download_available`) as `is_download_available`, mso.`is_pickup_mode`';
        ...
        $pickup_mode = (int)Configuration::get('MBESHIPPING_PICKUP_MODE');
        if($pickup_mode) {
    ...
    protected function processBulkRetrievePickupManifest()
    {
        ...
        if (empty($manifestsPdf) || file_put_contents($outputPdfPath, $outputPdf) === false) {
            $this->errors[] = $this->module->l('Download shipping manifests - error writing to file', 'AdminMbeShippingController') . ': ' . $outputPdfPath;
            return;
        }
